### 并发编程
Python中实现并发编程的方案有三种：**多线程**、**多进程**和**异步I/O**。
...
1. **多线程**并发
python提供Thread类并且辅以Lock、Condition、Event、Semaphore和Barrier。有[GIL(全局解释器锁)](https://www.cnblogs.com/ArsenalfanInECNU/p/9968621.html)来防止多个线程同时执行本地字节码，这个锁对于CPython是必须的，因为CPython的内存管理并不是[**线程安全**](https://blog.csdn.net/csdnnews/article/details/82321777)的，因为GIL的存在多线程并不能发挥**CPU的多核特征**。
> 说明：介绍GIL的链接中，也有说到线程安全相关知识；而所给线程安全链接是在java语言下的情况，不过其介绍比较易懂。

___
> 面试题：进程和线程的区别和联系？
> 
> 进程 - OS分配内存的基本单位 - 一个进程可以包含一个或多个线程
> 
> 线程 - OS分配CPU的基本单位
>
> 并发编程（concurrent programming）
>
> 1. 提升执行性能 - 让程序中没有因果关系的部分可以并发执行
>
> 2. 改善用户体验 - 让耗时的操作不会造成程序的假死
___
...
>
> 以下情况需要使用多线程：
> a. 程序需要维护许多共享的状态（尤其是可变状态），**python中的list、dict、set都是线程安全的**，所以使用线程而不是进程维护共享状态的代价相对较小。
>
> b. 程序会花费大量的时间在I/O操作上，没有太多并行计算的需求且不需要占用太多的内存。
>
>
> 以下情况会需要使用多进程：
>
...
>
> b. 程序的输入可以并行的分成块，且可以将运算结果合并。
> 
> c. 程序在内存使用方面没有任何限制且不强依赖于I/O操作，如读写文件、套接字等。
...
Python中有一个名为`aiohttp`的三方库，它提供了异步的HTTP客户端和服务器，这个三方库可以跟`asyncio`模块一起工作，并提供了对`Future`对象的支持。Python 3.6中引入了async和await来定义异步执行的函数以及创建异步上下文，**在Python 3.7中它们正式成为了关键字**。   
